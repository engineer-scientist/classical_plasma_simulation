/* ============================================================================
 *  plasma_sim.c  --  Classical (Newtonian) plasma simulation, SERIAL version
 * ============================================================================
 *
 ...
 *
 *  Data layout (Structure-of-Arrays) is chosen so the O(N^2) force kernel --
 *  the expensive part and the natural target for later multi-core (OpenMP)
 *  parallelisation -- is a simple, cache-friendly double loop.
 *
 *  Newton's third law: the Coulomb force particle j exerts on particle i is
 *  equal and opposite to the force i exerts on j, so we only iterate over
 *  pairs with j > i (the "upper triangle" -- half of the NP*NP matrix) and add
 *  the force to i and subtract it from j.  This halves the pair work.  (See the
 *  note in compute_forces(): this same optimisation must be revisited when the
 *  loop is parallelised, because two threads could then update particle j at
 *  once.)
 *
 ...
 *  Run:    ./plasma_sim [N] [nsteps] [seed]
 *          (all three are optional and fall back to the defaults below)
 *
 *  ------------------------------------------------------------------------- */
...
static int    N           = 1000;    /* particles PER species (NP = 2*N)      */
...
static long   NSTEPS      = 10000;    /* number of time steps T                */
static int    SAVE_STRIDE = 10;      /* save a trajectory frame every k steps */
static double TEMP        = 1.0e4;   /* plasma temperature [K] (10^5 K or ~8.6 eV is hot  *
                                      * enough to keep it ionised)            */
static double BX          = 0.0;     /* external magnetic field [T] ...       */
static double BY          = 0.0;
static double BZ          = 0.0;     /* ... if dominant along z -> gyration in xy */
static double EX          = 1.0e4;   /* external electric field [V/m] ...      *
 ...
 *
 *  Only pairs with j > i are visited (upper triangle) and Newton's third law
 *  is used to update both particles -- this is the "store/compute only half of
 *  the pairwise values" optimisation from the problem statement.
 *
 *  >>> PARALLELISATION NOTE (for the future multi-core version) <<<
 *  The two-sided update  fx[j] -= ...  makes this loop unsafe to naively split
 *  across threads (two threads may hit the same j).  When moving to OpenMP the
 *  usual options are: (a) drop the third-law trick and let each thread own a
 *  full row i (compute the force on i from ALL j != i, no writes to j -- 2x the
 *  flops but embarrassingly parallel), or (b) keep the half-loop with per-thread
 *  private force buffers that are reduced at the end.  This serial version keeps
 *  the cheaper half-loop.
 * ------------------------------------------------------------------------- */
static void compute_forces(int NP)
{
    for (int i = 0; i < NP; ++i) { fx[i] = fy[i] = fz[i] = 0.0; }
    ...
    for (int i = 0; i < NP; ++i) {
        ...
        for (int j = i + 1; j < NP; ++j) {
            ...
            fxi     += Fx;  fyi     += Fy;  fzi     += Fz;   /* force on i */
            fx[j]   -= Fx;  fy[j]   -= Fy;  fz[j]   -= Fz;   /* reaction on j */
...
{
    /* optional positional overrides: N, nsteps, seed */
    ...
    fprintf(stderr,
            ...
            N, N, NP, Lx, Ly, Lz, NSTEPS, DT, NSTEPS*DT, BX, BY, BZ,
    ...
    append_timing_row("serial", 1, NP, wall_total_s, force_s, drift_pct);
